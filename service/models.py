...
class Notification(orm.Model, ResourceAddUpdateDelete):
    ...
    notification_category_id=orm.Column(orm.Integer, 
                                            orm.ForeignKey('notification_category.id', ondelete='CASCADE'), 
                                            nullable=False)
    notification_category=orm.relationship('NotificationCategory', 
                                            backref=orm.backref('notifications', 
                                            lazy='dynamic', order_by='Notification.message'))
...
class NotificationCategorySchema(ma.Schema):
    ...
    name=fields.String(required=True, validate=validate.Lenght(3))
...
class NotificationSchema(ma.Schema):
    id=fields.Integer(dump_only=True)
    message=fields.String(required=True, validate=validate.Lenght(5))
    ttl=fields.Integer()
    ...
    def process_notification_category(self, data):
        ...
        if notification_category:
            if isinstance(notification_category, dict):
                notification_category_name=notification_category_name('name')
            else:
